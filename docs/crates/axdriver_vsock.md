...
`axdriver_vsock` 用来定义 vsock 驱动的统一接口。它既不是通用 socket API，也不是网络协议栈，而是把“面向 host/guest 通道的设备驱动”抽象成一组统一操作，让 `virtio-vsock` 之类的具体实现能被 `axdriver` 聚合层和 `axnet-ng` 上层消费。
...
- 向上被 `axdriver::prelude` 和 `axnet-ng` 使用。
...
这说明该 crate 不是独立的网络主线，而是 `axnet-ng` 可选能力的一部分。
...
- 让 `virtio-vsock` 这样的实现可以被 `axdriver` 聚合和 `axnet-ng` 消费。
...
### 3.3 分层关系总结
- 向下不耦合任何具体总线。
- 向上作为 `axnet-ng` 的一个设备能力来源。
- 真正的设备探测和 transport 建立仍由 `axdriver` 与 `axdriver_virtio` 负责。
...
## 6. 跨项目定位分析
### 6.1 ArceOS
ArceOS 通过 `axdriver` 和 `axnet-ng` 使用它，是当前仓库中唯一明确的主线落点。
