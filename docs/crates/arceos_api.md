...
`arceos_api` 是 ArceOS 官方的公共 API 门面层。它把底层 `ax*` 模块能力按领域重新组织成稳定的 `ax_*` 接口，使上层用户库、应用接口层和部分系统软件可以在不直接耦合大量内部模块的前提下访问内核能力。
...
### 1.3 宏驱动的 API 生成机制
`arceos_api` 最值得单独说明的是 `macros.rs` 中的两个宏：
...
因此，`arceos_api` 本身是薄的，但它承担了“把一组分散模块整理成统一上层接口”的体系化职责。
...
### 2.3 典型使用方式
对上层库或应用接口层来说，推荐优先依赖 `arceos_api` 而不是直接依赖多个底层模块：

```rust
use arceos_api::time::ax_monotonic_time;
use arceos_api::sys::ax_get_cpu_num;
...
如果稳定 API 不足，再显式使用 `arceos_api::modules` 中再导出的底层 crate。

## 3. 依赖关系图谱
```mermaid
graph LR
    axruntime["axruntime"] --> arceos_api["arceos_api"]
    axhal["axhal"] --> arceos_api
    axalloc["axalloc"] --> arceos_api
    axdma["axdma"] --> arceos_api
    axtask["axtask"] --> arceos_api
    axfs["axfs"] --> arceos_api
...
```toml
[dependencies]
arceos_api = { workspace = true, features = ["alloc", "multitask", "fs", "net"] }
```

### 4.2 使用与扩展原则
1. 上层应优先使用 `arceos_api` 暴露的稳定接口，不要一开始就直接依赖多个 `ax*` 内核模块。
...
## 5. 测试策略
### 5.1 当前测试形态
`arceos_api` 本身几乎没有独立的 crate 内测试，其正确性更多依赖 feature 组合编译与上层集成 smoke test。
...
### 5.4 覆盖率要求
- 对 `arceos_api`，比运行时覆盖率更重要的是“门面完整性覆盖率”。
