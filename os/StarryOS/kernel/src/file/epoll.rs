...
impl Epoll {
    ...
    pub fn poll_events(&self, out: &mut [epoll_event]) -> AxResult<usize> {
        let mut count = 0;
        // Collect LT (level-triggered) interests to re-queue after the loop.
        // This prevents the same interest from being processed multiple times
        // within a single poll_events call (which caused the same fd to fill
        // the entire output buffer).
        let mut requeue: VecDeque<Weak<EpollInterest>> = VecDeque::new();
        ...
        // Re-queue LT interests so they're checked on the next epoll_wait.
        if !requeue.is_empty() {
